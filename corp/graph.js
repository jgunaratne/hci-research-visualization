...
d3.tsv("people.tsv", function(e, data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (data[i].institution == "Palo Alto Research Center") {
        newTarget = 7;
    }
    if (data[i].institution == "Intel Corporation") {
        newTarget = 8;
    }
    if (data[i].institution == "Google Inc.") {
        newTarget = 9;
    }
    if (data[i].institution == "IBM Almaden Research Center") {
        newTarget = 10;
    }
    if (data[i].institution == "Philips Research") {
        newTarget = 11;
    }
    if (data[i].institution == "Fraunhofer Institute for Applied Information Technology") {
        newTarget = 12;
    }
    if (data[i].institution == "Italian National Research Council") {
        newTarget = 13;
    }
    if (data[i].institution == "Netherlands Organisation for Applied Scientific Research - TNO") {
        newTarget = 14;
    }
